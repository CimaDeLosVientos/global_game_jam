label sc_006_book_enemy:
    ...
    scene bg text forest 3 with fade
    nvl_narrator "{enf}Sabiendo como avanzar de forma segura continuaron su aventura adentrándose aún más en el bosque.{/enf}"
    nvl_narrator "{enf}El bosque cada vez era más verde y podría asemejarse a un lugar como otro cualquiera...{/enf}"
    nvl_narrator "{enf}Si no fuera porque las ramas de los árboles aún se entrelazaban entre sí, dejando que apenas pasara la luz,{/enf}"
    ...
    nvl_narrator "{enf}Pero ellas seguían dispuestas a averiguar su secreto.{/enf}"
    nvl_narrator "{enf}Tras unos cuantos árboles, se abría el bosque formando un claro...{/enf}"
    nvl_narrator "{enf}y en él se hallaba un gran árbol verde que parecía emanar luz propia.{/enf}"
    nvl_narrator "{enf}Cuando se acercaron y pudieron verlo mejor, descubrieron que la luz emanaba de un objeto que había en árbol.{/enf}"
    nvl_narrator "{enf}Se acercaron lentamente, como hechizadas y maravilladas por aquel árbol resplandeciente y el objeto que guardaba en él.{/enf}"
    ...
    "{i}Éstel sonrió ampliamente y se giró hacia el libro e intento abrirlo.{w} Sairina se acercó a ella y miró también.{\i}"
    ...
    girl "¿Y que hacemos?{w} ¡¿Qué hacemos?!"
